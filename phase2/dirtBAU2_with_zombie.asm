...
push es
pop ds
mov byte dl, [SHARED_MEM_OFFSET]
mov cl, 0x4
shl dx, cl
add dl, 0xA2 + 10 - interval - 2
pop ds

mov bx, ax
;brain1_col + 10 - interval - 2 + 0x200
mov dh, 0x02
mov word [bx + write_di_offset + 2], dx

mov bx, ax
mov dh, ah
add dh, 0x41
mov dl, 0x1c
mov word [bx + write_brain2_col + 1], dx
...
zombieTrap:
    %define brain1_col 0x05
	%define brain2_col 0x07
	%define di_offset brain1_col + 10 - interval - 2 + 0x200
	%define middle_opcode 0x218D
	%define steps_until_jmp 0x250
	%define jmp_dist 0x4000
	%define interval_jmp interval
	%define flag_offset 0x69
	%define ttl 0x33
	...
	start:
	lea bp, [di + flag_offset + 0x1E]
	and di, 0xFF00
	write_di_offset: ; + 2
	add di, 0X1111
	mov bx, [bp]
	add sp, jmp_dist + 2
	mov cx, steps_until_jmp
	cmp word [si], ttl
	je end
	inc word [si]
	xor si, si 
